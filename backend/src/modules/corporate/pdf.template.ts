type Contact = { first_name?: string; last_name?: string; company_role?: string; email?: string; contact_number?: string; contact_prefix?: string; system_role?: string; id?: string };

const esc = (s: unknown) => String(s ?? '').replace(/</g, '&lt;').replace(/>/g, '&gt;');

export function buildTimeline(corp: any): Array<{ action: string; timestamp: string; details: string }> {
  const out: Array<{ action: string; timestamp: string; details: string }> = [];
  ...
  let latestRejection: { ts: string; reason: string; reviewer: string; submitter: string } | null = null;

  const contacts: Contact[] = Array.isArray(corp?.contacts) ? corp.contacts : [];
  const primary = contacts[0];

  const nameOf = (c?: Contact) => ((c?.first_name || '') + ' ' + (c?.last_name || '')).trim();
  const roleOf = (c?: Contact) => c?.company_role || '';

  const deriveSubmitter = (fromStatus?: string | null) => {
  ...
  for (const log of logs) {
    ...
    if (log.to_status === 'Amendment Requested') {
      ...
      (note && /amendment\s+approved\s+by\s+crt\s+team/i.test(note)) ||
...
export function buildAgreementHtml(corp: any): string {
  ...
  const contacts: Contact[] = Array.isArray(corp?.contacts) ? corp.contacts : [];
  const primary = contacts[0];
  const second = (corp as any).secondary_approver as Contact | undefined;
  ...
  li{ margin-bottom: 4px; }
    ...
    <div class=\"grid\">
      ...
      <div class=\"row\" style=\"grid-column:1 / span 2\"><b>Office Address:</b> ${esc((corp as any).office_address1 || '')}${(corp as any).office_address2 ? `, ${esc((corp as any).office_address2)}` : ''}</div>
      <div class=\"row\"><b>City:</b> ${esc((corp as any).city || '')}</div>
      <div class=\"row\"><b>State:</b> ${esc((corp as any).state || '')}</div>
      <div class=\"row\"><b>Postcode:</b> ${esc((corp as any).postcode || '')}</div>
      <div class=\"row\"><b>Country:</b> ${esc((corp as any).country || '')}</div>
      <div class=\"row\"><b>Website:</b> ${esc((corp as any).website || '')}</div>
      <div class=\"row\" style=\"grid-column:1 / span 2\"><b>Account Note:</b> ${esc((corp as any).account_note || '')}</div>
    </div>
    
    <h2>Commercial Terms</h2>
    <div class=\"grid\">
      <div class=\"row\"><b>Agreement From:</b> ${esc((corp as any).agreement_from || '')}</div>
      <div class=\"row\"><b>Agreement To:</b> ${esc((corp as any).agreement_to || '')}</div>
      <div class=\"row\"><b>Credit Limit:</b> ${esc((corp as any).credit_limit || '')}</div>
      <div class=\"row\"><b>Credit Terms:</b> ${esc((corp as any).credit_terms || '')}</div>
      <div class=\"row\"><b>Transaction Fee:</b> ${esc((corp as any).transaction_fee || '')}%</div>
      <div class=\"row\"><b>Late Payment Interest:</b> ${esc((corp as any).late_payment_interest || '')}%</div>
      <div class=\"row\"><b>White Labeling Fee:</b> ${esc((corp as any).white_labeling_fee || 'N/A')}</div>
      <div class=\"row\"><b>Custom Feature Fee:</b> ${esc((corp as any).custom_feature_fee || '')}</div>
    </div>

...
export class CorporateService {
    ...
    async create(corporateData) {
        ...
        const corporateInsertData = {
            ...
            pinned: false,
            created_at: sql `date_trunc('second', now() AT TIME ZONE 'Asia/Kuala_Lumpur')::timestamp(0)`,
            updated_at: sql `date_trunc('second', now() AT TIME ZONE 'Asia/Kuala_Lumpur')::timestamp(0)`,
        };
    ...
    async updateStatus(id, status, note) {
        ...
        if (hasStatusChange || hasNote) {
            await this.addInvestigationLog(id, {
                timestamp: sql `(now() AT TIME ZONE 'Asia/Kuala_Lumpur')::text`,
        ...
        if (status === 'Cooling Period') {
            ...
            try {
                await this.db
                    .updateTable('corporates')
                    .set({
                    cooling_period_start: sql `(${coolingPeriodStart} AT TIME ZONE 'Asia/Kuala_Lumpur')::text`,
                    cooling_period_end: sql `(${coolingPeriodEnd} AT TIME ZONE 'Asia/Kuala_Lumpur')::text`,
                })
                    .where('uuid', '=', id)
                    .execute();
            }
    ...
    async expireStaleCorporatesDaily() {
        const thirtyDaysAgoIso = sql `(now() - interval '30 days' AT TIME ZONE 'Asia/Kuala_Lumpur')::text`;

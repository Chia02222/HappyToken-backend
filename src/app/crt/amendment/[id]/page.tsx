...
const CRTAmendmentReviewPage: React.FC = () => {
  ...
  const buildViewPair = () => {
    ...
    if (changedFields && typeof changedFields === 'object') {
      const amended: any = Array.isArray(original) ? [...original] : { ...original };
      Object.entries(changedFields).forEach(([k, v]) => { (amended as any)[k] = v; });
      return { original, amended };
    }
    // legacy fallback
    const legacyOriginal = amendmentData?.amendment_data?.original_data || {};
    const legacyAmended = amendmentData?.amendment_data?.amended_data || {};
    return { original: legacyOriginal, amended: legacyAmended };
  };
